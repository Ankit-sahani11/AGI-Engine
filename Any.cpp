...
struct ANY{// badal kar dod base banana hai 
    ...
    int *type;
    int size;
    const char* data_buffer;  // Asli values (6, 'k', etc.) isme raw bytes banker store hongi
    const int* offsets;       // Batayega ki kaun si value kis byte number par baithi hai
    static int index;
    int tempSize;
    int tempCap;
    ~ANY(){
    ...
    int getIndex(int num){
        int tempCap = 100;
        int my_Int_type[tempCap];
        for(int i = 0; i< size;i++){
           if(type[i] == My_Int){
               int Int_valus = *((int*)list[i]);
               my_Int_type[tempSize] = Int_valus;
               tempSize++; 
           }
        }
        sort(my_Int_type);
        int index = binarySearch(my_Int_type,num);
        return index;
    }
